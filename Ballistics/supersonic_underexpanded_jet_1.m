...
while M_blast>=1.265
    ...
    % P_b is pressure after blast
    p_b = P_b;
    %before expansion fan starts just at the end of tube
    %lets consider that the flow is just choked that is M=1
    %lets consider that Moving shock in tube is 2.1 and pressure after
    %shock is 4.9 bar
    %pressur in different regions is calculated and named as pr
    %_f denotes the conditions and properties in the front case after blast
    ...
    %b denotes mach angles in regions above projectile
    ...
    while a_f>f_f
        ...
        for z_f = g_f:n_f-1
            if 90-exp_fan_f(z_f)>angles_f
                ...
                % instead of using MOC made some manipulation such that we
                % get a solution possible
    ...
    %next point is to find the last expansion fan striking the flow
    if n_f >1
        if f_f 
    ...
    %next points is to find position of mach stem which I consider the
    %height of mach stem is nearly equal to diameter of the tube
    ...
    if n == 1
        x = fsolve(@(x) ntng(x,(n+f-hz)*def_par,b(f+1)+(f*def_par),P(tot_points+f,1),P(tot_points+f,2)),[0 0]);
        P((tot_points+f+1),1) = x(1);
        P((tot_points+f+1),2) = x(2);
    end
    ...
    %figure() 
    %dome is variable which denotes the horizontal length of slant of body
    x_co = x_co_f - length ;
    dome = cotd(ang)*D/2;
    %when projectile is inside the tube
    if x_co_f<=0
        tot_load_f = P_e*D*t/10;
    end
    flip = 0;
    %when projectle is between the first point of mesh on axis and tube end
...
%when projectile is between any two points in mesh in axis except last and
% even considered the wedge angle and its crossing the expansion fans 
    r_f = n_f+1;
    r0_f = n_f+1;
    j_f = 0;
    s_f = 0;
    ...
    % this step is to make unsteady dt is fps(frames per second)
    dt = 1/700000;%(in sec);
    ...
    time = time+dt;
    %as the blast velocity decreases just a random decrease in M-blast as
    %the attenuation pattern is not known in this case so random guess
    %blast_speed = 725.6*exp(-28.97*1000*(time))+583*exp(-0.753*1000*(time));
    if M_blast >=1.4
        M_blast = M_blast - 0.02;
    else 
        M_blast = M_blast - 0.00028;
    end
